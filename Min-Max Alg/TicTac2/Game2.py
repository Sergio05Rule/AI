...
class Game:
    ...
    def max_value(self, state):
        ...
        if self.depth == 0:
            print('Chiamo la funzione max sullo stato', state.board,'\n\n')
        ...
        for action in state.action():
            ...
            '''
            if self.depth == 0:
                print('Nuovo stato per MAX', new_state.board, 'stato iniziale', self.current_state.board)
                input('Avanti in max')
            '''

            # todo: self.current_state = result(action)
            #self.current_state = self.current_state.result(action)
            #state.board[action[0]][action[1]] = 'O'
    ...
    def min_value(self, state):
        ...
        for action in state.action():
            ...
            '''
            if self.depth == 0:
                print('Nuovo stato per MIN', new_state.board, 'stato iniziale', self.current_state.board)
                input('Avanti in min')
            '''

            # todo: self.current_state = result(action, X)
            #self.current_state = self.current_state.result(action)
            #state.board[action[0]][action[1]] = 'X'
    ...
    def min_max_decision(self, Human = False):
        ...
        while True:
            ...
            if self.current_state.player_turn == 'X':
                ...
                while True:
                    ...
                    if Human == False:
                        ...
                        start = time.time()
                        (m, move) = self.min_value(temp_state)
                        end = time.time()
            ...
            # If it's MAX turn (always controlled by AI)
                ...
                temp_state = self.current_state.copy()
                start = time.time()
                (m, move) = self.max_value(temp_state)
                end = time.time()
                print('Tempo elaborazione mossa: {}s'.format(round(end - start, 7)))
                print('La mossa pensata dalla cp1 è: ', move, ' con m = ', m)
                self.current_state = self.current_state.result(move)
